...
const Rasterizer = {
    ...
    copyOpaquePixelsClipped(sourcePixels, sourceWidth, sourceHeight, targetPixels, targetWidth, targetHeight, targetX, targetY) {
        const left = Bitwise_clampZero(targetX);
        const right = Bitwise_clampHigh(targetX + sourceWidth, targetWidth);
        if (left > right) return;

        const top = Bitwise_clampZero(targetY);
        const bottom = Bitwise_clampHigh(targetY + sourceHeight, targetHeight);
    ...
    fillRectangleClipped(pixels, rasterWidth, rasterHeight, rectangleX, rectangleY, rectangleWidth, rectangleHeight, color) {
        let left = Bitwise_clampZero(rectangleX);
        let right = Bitwise_clampHigh(rectangleX + rectangleWidth, rasterWidth);
        if (left >= right) return;

        let top = Bitwise_clampZero(rectangleY);
        let bottom = Bitwise_clampHigh(rectangleY + rectangleHeight, rasterHeight);
        if (top >= bottom) return;
